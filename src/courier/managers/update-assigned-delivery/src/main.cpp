...
int main() {
    ...
    while(1) {
        ...
        // Convert request
        try{
            delivery_purchase = DeliveryPurchase::update_from_stream(reply, 0, 0);
        }
        ...
        query = "SELECT * FROM AssigndDelivery WHERE delivery_code = " + std::string(delivery_purchase->delivery_code);
        ...
        query = delivery_purchase->to_update_query();

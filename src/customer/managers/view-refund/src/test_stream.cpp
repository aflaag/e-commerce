...
#define WRITE_STREAM2 "view-refund-in"
#define READ_STREAM2 "view-refund-out"
...
int add_to_stream() {
    ...
    reply = RedisCommand(c2r, "XADD %s * client_id 1 product 1", WRITE_STREAM2);
...
int read_from_stream(){
    ...
    initStreams(c2r, READ_STREAM2);

    reply = RedisCommand(c2r, "XREADGROUP GROUP main customer BLOCK 0 COUNT 1 STREAMS %s >", READ_STREAM2);

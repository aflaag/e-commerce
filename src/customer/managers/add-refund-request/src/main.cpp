...
int main() {
    ...
    while(1) {
        ...
        sprintf(query, "INSERT INTO RefundRequest (request_instant, refund_state, assigned_delivery) VALUES (\'%s\', \'%s\', \'%s\')", 
                        refund->request_instant, refund->state, refund->assigned_delivery);
